...
school_ids = ['12345', '23456', '34567', '45678', '67890', '11234']
...
for school_id in school_ids:
    cursor.execute("SELECT user_id, role FROM users WHERE school_id=?",(school_id,))
    users = cursor.fetchall()
    cursor.execute("SELECT book_id, title FROM books WHERE school_id=?",(school_id,))
    library = cursor.fetchall()
    selected_books = random.sample(library, 80)
    for user in users:
        # Generate a random number of reports for each user
        if user[1] == "Admin" or user[1]=="School Admin":
            continue
        ...
        for _ in range(num_reports):
            ...
            cursor.execute("INSERT INTO reports (report_id, user_id, book_id, title, date, issue_date, issue, school_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                        (report_id, user[0], book[0], book[1], date.strftime("%Y-%m-%d"), issue_date, issue, school_id))
            connection.commit()
